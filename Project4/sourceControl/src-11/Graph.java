...
public class Graph {
    ...
    I modified the algorithm to end prematurely once the terminal node is not reachable. This happens
    when every transport has been relabeled. The output is correct because the maximum flow is the 
    excess in the terminal. So when the terminal node is not reachable (every other node has height of
    at least 2) there is no need to further run the program. Also, adding nodes to the active node queue
    is modified for bipartite matching. Furthermore, in each iteration the terminal node is checked first
    to improve runtime.
    */
